...
BANNED_WORDS = [
    ...
    "пидорасам","черножопым","чуркобесам","аулавецам","рашистам","инцелам","негритенокам", "жидам",
    "куколдам","черножопому","куколду","аулавецу","рашисту","инцелу","негритенку", "жиду",
    "черномазому","дауну","педикам","пидорасу","негру","кацапу","нигеру", "ниггеру",
]
...
while True:
    ...
    if msg == "!tg":
        ...
        sock.send(f"PRIVMSG {CHANNEL} :Новый Человек-паук 2012, фантастика, США, 131 мин, 12+ 🕷\r\n".encode())
    elif msg == "!кино":
        sock.send(f"PRIVMSG {CHANNEL} :Новый Человек-паук 2012, фантастика, США, 131 мин, 12+ 🕷\r\n".encode())
